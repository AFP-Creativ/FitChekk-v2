//
//  OutfitCreationView.swift
//  FitChekk
//
//  View for creating manual outfits with drag-and-drop item selection
//
...
@MainActor
struct OutfitCreationView: View {
    @Bindable var store: StoreOf<OutfitCreationFeature>
    ...
    private var outfitCanvasSection: some View {
        VStack(spacing: Spacing.md) {
            ...
            if store.selectedItems.isEmpty {
                ...
                // Item Grid with Drag-to-Reorder
                itemGridView
            }
    ...
    private var emptyCanvasState: some View {
        VStack(spacing: Spacing.md) {
            ...
            Text("Tap the button below to add items from your wardrobe")
...
extension Binding {
    func defaultValue<T>(_ value: T) -> Binding<T> where Value == T?, T: Sendable {
        Binding<T>(
...
#Preview {
    OutfitCreationView(
        store: Store(initialState: OutfitCreationFeature.State()) {
